...
import com.abc.kzvideodownloader.model.ForceUpdate

import com.abc.kzvideodownloader.tasks.downloadFile.downloadID
import com.abc.kzvideodownloader.tasks.downloadVideo
import com.abc.kzvideodownloader.utils.PrefManager
...
//    }
    ...
    override fun onStart() {
        ...
        var version: String? = pref.getString("Version","")
        val database = Firebase.database
        val myRef = database.getReference("version")
        myRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if(dataSnapshot.exists()){
                var  c_version : String = dataSnapshot.getValue() as String
                    if (!version.equals(c_version)){
                        Log.e("VERSION REPORT","LATEST VERSION");
                    }else{

                        val builder = AlertDialog.Builder(this@MainActivity)
                        builder.setCancelable(false)
                        builder.setTitle("Update!!")
                        builder.setMessage("Please update to latest to get latest feature")
                        builder.setPositiveButton("Update") { dialog, which ->
                            startActivity(
                                Intent(
                                    Intent.ACTION_VIEW,
                                    Uri.parse("market://details?id=com.abc.kzvideodownloader")
                                )
                            )
                            dialog.dismiss()
                        }
